document.addEventListener('DOMContentLoaded', function() {
  ...
  if (loginFormElement) {
    loginFormElement.addEventListener('submit', function(e) {
      ...
      // Exibe o modal de confirmação de cadastro antes de redirecionar
      const modal = document.getElementById('cadastroConfirmModal');
      if (modal) {
        modal.style.display = 'flex';
        const btnIrPainel = document.getElementById('btnIrPainel');
        if (btnIrPainel) {
          btnIrPainel.onclick = function() {
            modal.style.display = 'none';
            window.location.href = '/painel_admin';
          };
        }
      } else {
        // fallback: se não houver modal, redireciona direto
        window.location.href = '/painel_admin';
      }
    });
  }
      ...
      setTimeout(() => {
          ...
          fetch('/forgot') // Corrigido para usar a rota correta
              ...
              .then(html => {
                  ...
                  // >>> Adicione esta linha para garantir o evento do submit:
                  setupForgotFormSubmit();
              })
              .catch(err => console.error(err.message));
      }, 600); // Tempo ajustado para a animação de saída do login
  }); // <-- Adiciona o parêntese e a chave faltantes aqui

// Função para atribuir evento ao formulário de recuperação de senha
function setupForgotFormSubmit() {
  ...
  if (forgotForm && !forgotForm.dataset.bound) {
    ...
    forgotForm.addEventListener('submit', function(ev) {
      ...
      const forgotPasswordForm = document.getElementById('forgotPasswordForm');
      if (forgotPasswordForm) forgotPasswordForm.classList.remove('show');
      setTimeout(() => {
        ...
        fetch('/sent')
          ...
          .then(html => {
            ...
            setTimeout(() => {
              ...
              // Inicializa eventos da caixa sent.html
              setupSentBoxEvents();
            }, 10);
          })
...
function setupSentBoxEvents() {
  ...
  if (backToLoginFromSent) {
    backToLoginFromSent.onclick = function(e) {
      e.preventDefault();
      sentBox.classList.remove('show');
      setTimeout(() => {
        const forgotPasswordContainer = document.getElementById('forgotPasswordContainer');
...
// Chame também setupForgotFormSubmit() no carregamento inicial, caso o formulário já esteja presente:
setupForgotFormSubmit();
