...
// Estação Meteorológica
// Removido: declarações duplicadas de estacaoContainer, estacaoExemplo, estacaoToggle, estacaoExpanded
if (estacaoContainer && estacaoExemplo) {
...
function renderValvulas() {
  ...
  // --- Controle dinâmico do scroll da caixa de válvulas ---
  if (valvulasExemplo) {
    // Altura de cada linha (aprox. altura da .valvula-box + margin/gap)
    const boxHeight = 240; // min-height da .valvula-box
    const rowGap = 24; // gap entre linhas
    const addBoxHeight = 38; // altura do .frame-4 (botão adicionar)
    const paddingTop = 32, paddingBottom = 24; // padding da caixa
    const maxHeight = 420; // altura máxima padrão da caixa

    // Quantas linhas cabem sem scroll?
    const availableHeight = maxHeight - paddingTop - paddingBottom - addBoxHeight;
    const maxRows = Math.floor((availableHeight + rowGap) / (boxHeight + rowGap));

    // Quantas linhas temos?
    const numRows = rows.length;

    if (numRows > maxRows) {
      valvulasExemplo.style.maxHeight = maxHeight + "px";
